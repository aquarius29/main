 ...
 */
positionList * AStar(const position * start, const position * end, int8_t map[][MAP_X])
{
	...
	for(y = -1; y < 2; y++) {
		for(x = -1; x < 2; x++, counter++) {
			...
			// the open list we create it
				...
				p_adjacentNode->heuristic = GetHeuristicCost(&p_adjacentNode->pos, end);
			}
			...
			if(!NodeInOpen(&neighborY, &neighborX, open)) {
				...
				// Sort the open list so the node with the smallest cost, including the
				// heuristic cost, is the first node
				for(sortCounter = 0; sortCounter < open->count; sortCounter++) {
					if((p_adjacentNode->totalCost + p_adjacentNode->heuristic)
					 < (open->list[sortCounter].totalCost + open->list[sortCounter].heuristic)) {
