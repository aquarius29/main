...
int main(void) {
	...
	int map[MAP_Y][MAP_X] =	// rows, columns
...
void AddNeighborsToOpen(node * current, nodeList * open, nodeList * closed,
		int map[][MAP_X]) {
	int8_t sortCounter, y, x, mapCost, counter;
	int32_t openListIndex, endNodeCost, neighborY, neighborX;
	node adjacentNode, * p_adjacentNode, tempNode;
	...
	for(y = -1; y < 2; y++) {
		for(x = -1; x < 2; x++, counter++) {
			...
			if(!NodeInOpen(&neighborY, &neighborX, open)) {
				...
				// is the first node
				for(sortCounter = 0; sortCounter < open->count; sortCounter++) {
					if(p_adjacentNode->totalCost < open->list[sortCounter].totalCost) {
...
positionList * CreateFinalList(nodeList * closed, const position * goal, const position * drone) {
	...
	final = malloc(sizeof(positionList));
	if(final == NULL) {
		printf("Could not allocate memory for final list\n");
		return NULL;
	}
	final->list = malloc(closed->count * sizeof(position));
	...
	for(counterUp = 1; counterUp < closed->count - 1; counterUp++)
	{
		...
		&& final->list[counterUp].x == drone->x) {
			final->list[counterUp].y = drone->y;
			final->list[counterUp].x = drone->x;
			final->count++;
			break;
		}
...
void AddNodeToOpen(node * current, nodeList * open) {
	...
	open->list[open->count - 1].pos.x = current->pos.x;
	open->list[open->count - 1].pos.y = current->pos.y;
	open->list[open->count - 1].previous = current->previous;
	open->list[open->count - 1].totalCost = current->totalCost;
}
...
node * AddNodeToClosed(node * current, nodeList * closed) {
	...
	closed->list[closed->count - 1].pos.x = current->pos.x;
	closed->list[closed->count - 1].pos.y = current->pos.y;
	closed->list[closed->count - 1].previous = current->previous;
	closed->list[closed->count - 1].totalCost = current->totalCost;
...
void RemoveNodeFromOpen(node * current, nodeList * open) {
	...
	for(counter = 1; counter < open->count; counter++) {
		open->list[counter - 1].pos.x = open->list[counter].pos.x;
		open->list[counter - 1].pos.y = open->list[counter].pos.y;
		open->list[counter - 1].previous = open->list[counter].previous;
		open->list[counter - 1].totalCost = open->list[counter].totalCost;
	}
